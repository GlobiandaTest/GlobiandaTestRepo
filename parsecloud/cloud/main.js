...
Parse.Cloud.define("testEmail", function(request, response) {
  ...
  var template = fs.readFileSync("cloud/templates/accountEmail.ejs");
  ...
  query.find({
    success: function (results) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        var comida = {};
        comida.name = object.get('name');
        comida.description = object.get('description');
        comida.seller = object.get('seller');
        comida.price = object.get('price');
        comida.img = object.get('image').url();
        comida.url = 'http://www.google.com';
        console.log("comida.img es igual a :" + comida.img + " ;)");
        comidas[i] = comida;
      };
 
      var html =  ejs.render(template,{date:today,comidas:comidas});
      ...
      queryUser.find({
        success: function (resultusers) {
          ...
          for (var j = 0; j < resultusers.length; j++) {
            var userdata = resultusers[j];
            destination[j]={email: userdata.get('email'), name: userdata.get('fullname')};
          };
    ...
    /*var comidas = [
      {
        name: 'Choripan',
        desc: 'Lo mejor de lo mejor en el mundo, nunca m?vas querer otro choripan.',
        seller: 'Maru Botana',
        price: '30',
      ...
      {
        name: 'Milanesa',
        desc: 'Lo mejor de lo mejor en el mundo, nunca m?vas querer otra milanesa.',
      ...
      },
      {
        ...
        desc: 'Lo mejor de lo mejor en el mundo, nunca m?vas querer otra cabeza de chancho.',
        seller: 'Narda Lepes',
        price: '30',
